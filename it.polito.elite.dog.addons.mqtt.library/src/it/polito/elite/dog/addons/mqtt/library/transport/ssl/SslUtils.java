 ...
 * 
 */
package it.polito.elite.dog.addons.mqtt.library.transport.ssl;
...
public class SslUtils {
     ...
     */
    public static SSLSocketFactory getSslSocketFactory(String caCrt, String crt, String key,
            String password) throws IOException, KeyStoreException, NoSuchAlgorithmException,
            CertificateException, UnrecoverableKeyException, KeyManagementException {

        char[] passwdChars = password != null && password.length() > 0 ? password.toCharArray()
                : "".toCharArray();
    ...
    /**
     * Creates a SSLSocketFactory using only the CA information required to
     * authenticate the server.
     * 
     * @param caCrt
     *            The CA file (path)
     * @return the CA only SSLSocketFactory
     */
    public static SSLSocketFactory getCaOnlySslSocketFactory(String caCrt)
            throws IOException, KeyStoreException, NoSuchAlgorithmException, CertificateException,
            UnrecoverableKeyException, KeyManagementException {
        // prepare a certificate converter
        JcaX509CertificateConverter certConverter = new JcaX509CertificateConverter();
        certConverter.setProvider("BC");
        ...
        // generate a key store for the CA
        KeyStore caKs = KeyStore.getInstance(KeyStore.getDefaultType());
        caKs.load(null, null);
        caKs.setCertificateEntry("ca-certificate", certConverter.getCertificate(caCert));

        // initialize a TrustManagerFactory with the key store holding the CA
        TrustManagerFactory tmf = TrustManagerFactory
                .getInstance(TrustManagerFactory.getDefaultAlgorithm());
